...
async def test_compile_with_predict_instances():
    ...
    lm = DummyLM([{"input": "Initial thoughts", "output": "Finish[blue]"}])
    dspy.settings.configure(lm=lm)

    # Initialize BootstrapFewShot and compile the student
    bootstrap = BootstrapFewShot(metric=simple_metric, max_bootstrapped_demos=1, max_labeled_demos=1)
    compiled_student = await bootstrap.compile(dspy.settings, student, teacher=teacher, trainset=trainset)
...
async def test_bootstrap_effectiveness():
    ...
    prediction = await compiled_student(settings=dspy.settings, input=trainset[0].input)
    assert prediction.output == trainset[0].output


async def test_error_handling_during_bootstrap():
    """
    ...
    class BuggyModule(dspy.Module):
        ...
        def forward(self, settings, *args, **kwargs):
    ...
    with pytest.raises(RuntimeError, match="Simulated error"):
        await bootstrap.compile(dspy.settings, student, teacher=teacher, trainset=trainset)


async def test_validation_set_usage():
    """
